...
- §2.2 marker-split's closure-(b) branch is precisely the ellipsis-restoration break-license that SHOULD re-break a closure-eligible amplification beat (one whose gapped finite verb is restorable from the prior parallel clause).
...
7. **Elided-predicate `(and|or) (all) this` matrix** (codified 2026-05-15): analogous to R6 Exclusion 5. When the *that*-marked advcl's head is the PRON `this` (lemma=`this`, upos=PRON, deprel=`conj`) that is itself `conj` of a VERB/AUX on the PRIOR v2-mine line, and the head's line begins with a CCONJ fragment-marker, R7's split-mandate YIELDS to §1.5 M4. Closed lexical signature: PRON `this` only. Helper `is_elided_this_matrix` shared with R6 validator. 1 corpus instance under R7 (Helaman 6:22 `and this / that they might distinguish a brother...`); 4 sibling cases governed by R6 Exclusion 5.
...
11. Cross-line attachment where `child_subtree_min_line ≤ head_line` (backward attachment) → REVIEW-REQUIRED (UD-parser subtree fan-out artifact, per Alma 24:26 line-map-FP precedent codified 2026-05-12). **PUNCT-exclusion qualifier:** when computing `child_subtree_min_line`, PUNCT tokens MUST be excluded — UD parsers attach the comma at head-line-end to the relative's verb as `punct`, which would falsely flag legitimate cross-line restrictives as "backward attachment." Use `min(line for tok in subtree(child) if tok.upos != "PUNCT")`. Codified post-Alma 31:22 false-suppression discovery 2026-05-12 (24 corpus cases recovered by the qualifier).
...
For per-change audit trail, see `git log -- 1-method/colometry-canon.md` (commit messages contain rationale, audit-dispatch evidence, and retraction precedents). For verbatim discussion logs, see the JSONL session transcripts at `~/.claude/projects/.../*.jsonl`. The canon prose above is the current method; this canon does not maintain a chronological narrative log inside itself.
