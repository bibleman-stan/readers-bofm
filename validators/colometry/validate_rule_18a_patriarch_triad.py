...
def scan_file(path: Path) -> list[dict]:
    """Scan v2-mine file for R18a violations (triad split across lines).

    Note: the subject-orphan predicate-completion pattern (when the triad
    is the grammatical subject of a finite-verb predicate orphaned on the
    following line) is governed by M4-BoFM-1 (canon §5 M4-BoFM-1) and
    detected by `validate_m4_bofm_1_subject_orphan.py`. R18a is scoped to
    the triad-internal token sequence only.
    """
    violations = []
