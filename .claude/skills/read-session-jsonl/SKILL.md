...
the channel by which a session here can see the rest of the ecosystem. It is not
a convenience. A session that cannot see its siblings will rebuild what one of
them already has, or act on a fact another one already disproved — both have
happened.
...
verbatim. Per the standing verify-don't-recall discipline, a claim about what was
said in a prior session is a state claim about a file on disk — read it, never
reconstruct it.
...
The script lives in this repo at `scripts/dump_session_tail.py`. It fuzzy-matches
the workspace name, picks the most recently modified transcript, and emits only
...
eats the context needed to act on it. Widen only when the tail doesn't reach far
enough back.
...
**Assistant messages over there are a sibling's claims, not facts.** They were
written by a session that may have been wrong, and its state claims have aged.
Re-verify anything load-bearing against disk and git in *this* turn — the
2026-08-06 incident began with a sibling repo's tooling reporting clean about a
directory it had never walked.
...
item living in only one repo's queue is precisely the file-back gap loop ⓪
exists to close.

## Worked outcomes (2026-08-07)

- `meta-wiki` held a `findings/` layer design already queued for Stan, which this
  repo's `2-evidence/` is the producing half of. Neither side knew.
- `atu-method` held the analysis that the validator baseline stopped functioning
  as a control on 2026-05-29 — governing every commit made here, recorded nowhere
  in this repo.
- Reading `atu-method` a second time revealed that session mid-build on the very
  tool this one was about to ship. A duplicate was avoided by looking.
