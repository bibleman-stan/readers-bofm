...
Usage (needs atu-method on PYTHONPATH, repo .venv):
  PYTHONPATH=../atu-method .venv/Scripts/python.exe 5-machinery/scripts/bofm_generate.py 1nephi 1
  (book, chapter; omit chapter for whole book)

  --stage v1   render the framework §3 v1 CLAUSE layer instead: bare clause atoms
               with the binding rules skipped, written to data/text-files/v1-clause/.
               A parallel view — nothing builds or deploys from it. Default is v2.
"""
import json
import os
import re
...
# ALSO the package parent: parse_repair.py does `import scripts.bofm_generate`,
# which needs the directory CONTAINING `scripts` on the path. That used to be the
# repo root and became 5-machinery/ in the 2026-08-08 move.
sys.path.insert(0, str(REPO / "5-machinery"))
...
def _colon_closure_eligible(toks, j):
    ...
    under framework §2.2(ii)? TRUE iff EITHER:
      ...
      (b) it is forward-closed by ellipsis-restoring a GAPPED FINITE VERB from the
    ...
    if own_subj and _is_finite(head_verb, toks):
        return True
    # (b) forward-closed by ellipsis-restoring a GAPPED FINITE VERB from the prior
...
def verse_atu_lines(verse_text, sentences, stage="v2"):
    ...
    `stage` selects how far down framework §3's pipeline to run:

      "v2" (default, DEPLOYED) — clause atoms + the full `_rule_passes` binding
            layer (R29, yea-B, AICTP, short-answer peel, forward-frame bind, §2.2
            marker split). This is what ships; every existing caller gets it.
      "v1" (PARALLEL VIEW, ships nothing) — the bare `bofm_v1_fabric.clause_atoms`
            segmentation with `_rule_passes` SKIPPED. framework §3's v1 stage:
            parse-derived clauses before any ATU binding merges them.

    What v1 still runs, and why (these are RENDERING, not binding — none of them
    merges two clause atoms into one ATU on grammatical grounds):
      - line-final-CCONJ travel: moves a trailing "and"/"or" onto the next line so
        the coordinator leads its conjunct. Same line count either way; this pass
        exists *for* fabric-level conj splits, so v1 needs it more than v2 does.
      - backward punctuation attachment (Stan's convention; punctuation has zero
        force per framework §2.1, so this can never be an ATU boundary).
      - lone-leader carry and the no-alnum-content sweep: a line that is ONLY
        "and" / "that" / "--" is not a clause at all, so leaving it standing would
        put non-clauses into a clause-level view and corrupt any length statistic
        taken over it. Both are degenerate-line guards, not clause merges.
    """
...
def generate(book, chap=None, stage="v2"):
    """Render a book (or one chapter) as verse-marker + one line per unit.

    stage="v1" renders the framework §3 v1 clause layer as a PARALLEL view. It
    also drops the two v2-only judgment layers, which have no meaning below the
    binding stage: `overrides.json` (LLM-adjudicated ATU residuals, keyed to v2
    line shapes) and `cross-verse-merges.json` (an editorial Layer-3 override
    that additionally inserts a superscript marker char, so carrying it would
    break v1's character-parity with v0)."""
    verses = read_v0(book)
    parsed = parse_book(book)
    cross_verse = {} if stage == "v1" else _load_cross_verse_merges()
    out = []
    ...
    for ki, (c, v) in enumerate(sorted_keys):
        ...
        ref_str = f"{book} {c}:{v}"
        out.append(f"{c}:{v}")
        ov = None if stage == "v1" else _apply_override(verses[(c, v)], ref_str)
        atu_lines = ov if ov is not None else verse_atu_lines(verses[(c, v)], parsed.get((c, v), []), stage)
...
V1_OUT_DIR = REPO / "data" / "text-files" / "v1-clause"
...
def _stage_target(book, stage):
    """Where a stage's render is written. v2 -> the deployed booklist path; v1 ->
    the same basename under data/text-files/v1-clause/, a PARALLEL layer nothing
    builds or deploys from (build_book.py + the validators read booklist.txt, so
    they never see this directory)."""
    v2 = _v2_target(book)
    return v2 if stage != "v1" else V1_OUT_DIR / v2.name
...
def write_book(book, stage="v2"):
    ...
    lines = generate(book, stage=stage)
    path = _stage_target(book, stage)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path


def build_all(stage="v2"):
    ...
    aligned CoNLL-U. Run `5-machinery/scripts/bofm_bidir_gate.py` after to measure. This is
    ...
    Under --stage v1 the CoNLL-U emit is SKIPPED: v0-cache-conllu is the deployed
    v2 substrate the validators score against, and a parallel view must not
    rewrite it."""
    for b in BOOKFILE:
    ...
    print(f"DONE: {len(BOOKFILE)} books -> "
          f"{V1_OUT_DIR if stage == 'v1' else OUT_DIR}")


def _pop_stage(argv):
    """Pull `--stage v1` / `--stage=v1` out of argv (default v2) so the positional
    book/chapter parsing below is unchanged for every existing caller."""
    stage = "v2"
    for i, a in enumerate(argv):
        if a == "--stage" and i + 1 < len(argv):
            stage = argv[i + 1]; del argv[i:i + 2]; break
        if a.startswith("--stage="):
            stage = a.split("=", 1)[1]; del argv[i]; break
    if stage not in ("v1", "v2"):
        sys.exit(f"--stage must be v1 or v2, got {stage!r}")
    return stage


def main():
    argv = sys.argv[1:]
    stage = _pop_stage(argv)
    if "--all" in argv:
        build_all(stage)
        return
    if "--write" in argv:
        argv.remove("--write")
        book = argv[0]
        print(f"wrote {write_book(book, stage)}")
        return
    book = argv[0] if argv else "1nephi"
    chap = int(argv[1]) if len(argv) > 1 else None
    print("\n".join(generate(book, chap, stage)))
