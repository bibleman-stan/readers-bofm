...
Verifies STRUCTURAL alignment between §5 rule entries and validator code:
  1. Validator file presence — backtick-quoted ``5-machinery/validators/.../.py`` paths
  ...
  2. Closed-list presence — every UD-signature-named closed-list appears as
     ...
     ``5-machinery/validators/_shared/`` (protocol commit 49ee753 fix #1); cross-rule
     references resolved elsewhere in ``5-machinery/validators/`` count as PRESENT
  3. UD signature field consistency — deprels and lemmas from the signature
     appear as string literals or constants anywhere in ``5-machinery/validators/``
...
Usage:
    python3 5-machinery/validators/canon/check_canon_alignment.py
"""
...
def extract_validator_paths(body: str) -> list[str]:
    ...
    backtick-quoted ``5-machinery/validators/.../.py`` patterns. Catches both:
      ...
      (b) inline-prose detector references anywhere in the rule body (e.g.,
          "the detector at ``5-machinery/validators/.../.py`` covers this")
    ...
    """
    paths = re.findall(r'`(5-machinery/validators/[^`]+\.py)`', body)
...
def audit_rule(rid: str, title: str, body: str) -> dict:
    ...
    # 5-machinery/validators/_shared/ are legitimately PRIMARY presence, not cross-rule. A
    ...
    shared_src = "\n".join(
        ...
        for p in REPO.glob("5-machinery/validators/_shared/**/*.py")
    ...
    all_validators_src = "\n".join(
        ...
        for p in REPO.glob("5-machinery/validators/**/*.py")
    ...
    for n in canon_names:
        if n in primary_src:
            continue   # PRESENT in named detector OR in 5-machinery/validators/_shared/
    ...
    # like "advcl" / "ccomp", and a match anywhere in the 5-machinery/validators/ tree
