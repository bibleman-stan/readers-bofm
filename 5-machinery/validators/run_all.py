...
Discovers every `validate_*.py` script under `5-machinery/validators/syntax/` and
`5-machinery/validators/colometry/`, runs each as a subprocess, collects exit code
...
Usage:
    python3 5-machinery/validators/run_all.py
    python3 5-machinery/validators/run_all.py --verbose       # forward --verbose to each
    python3 5-machinery/validators/run_all.py --staged-only   # only check files staged for commit (for pre-commit hook)
"""
...
BASELINE_PATH = REPO_ROOT / "5-machinery" / "validators" / ".baseline.json"
...
def main():
    ...
    ap.add_argument("--baseline-check", action="store_true",
                    help="Compare current counts to 5-machinery/validators/.baseline.json; exit 1 if any rule's count increased.")
    ...
    if not validators:
        print("No validators found under 5-machinery/validators/syntax/ or 5-machinery/validators/colometry/")
        return 0
