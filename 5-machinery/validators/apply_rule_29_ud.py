"""
Apply Rule 29 (Bare Infinitival Orphan Integrity) STRONG-MERGE findings to v2-mine.

For each STRONG-MERGE-CANDIDATE: governor on line N, line-initial infinitival
`to`+VERB on line N+1. Merge by joining the two lines.

Re-runs the detector internally for fresh line numbers.

Usage:
    python 5-machinery/validators/apply_rule_29_ud.py            # dry-run
    python 5-machinery/validators/apply_rule_29_ud.py --apply    # write
"""
...
from validators.colometry.validate_rule_29_ud import scan_book, BOOKS  # noqa: E402
...
def main() -> int:
    ...
    for book_id in BOOKS:
        ...
        for v in vs:
            ...
            gov_idx = v["gov_line"] - 1
            mark_idx = v["mark_line"] - 1
            if mark_idx != gov_idx + 1:
                skipped.append((book_id, v["gov_line"], "non-adjacent-gap"))
                continue
            by_file.setdefault(v2_path, []).append(gov_idx)

    plan_count = sum(len(set(items)) for items in by_file.values())
    print(f"Rule 29 STRONG-MERGE detected: {total_strong}")
    ...
    print(f"\nTotal Rule 29 merges applied: {total_applied}")
