#!/usr/bin/env python3
"""
Sense-Line Reformatter v8: Relaxed for CSS Wrap-Indent
=======================================================
...
1. Run this script against the uploaded .txt file.
   Command: python3 senseline_reformat_v8.py INPUT.txt OUTPUT.txt
...
v8 CHANGES FROM v7:
  - Relaxed cosmetic thresholds since CSS wrap-indent now handles overflow:
    M6:  58 → 72
    M8:  45-70 → 60-90
    M9:  >70 → >85
    M10: 70 → 90 (main threshold), 75 → 100 (no-comma preps)
    M15: ≥50 → ≥65
  - Added M18: List-stacking for "X, and Y, and Z" parallel triads
  - All semantic passes (M0-M5, M7, M11-M14, M16, M17) unchanged
...
Rules applied in order:
  ...
  M4:  Insomuch split
  M5:  Vocative split
  M6:  According-to split (lines >72)                          [was >58]
  M7:  Speech verb + content word split (lines ≥48)
  M8:  ", and [subject pronoun]" split (lines 60-90, non-triad) [was 45-70]
  M9:  Subject-predicate split at modal verbs (lines >85)       [was >70]
  M10: Long-line pattern-priority split (lines >90)             [was >70]
  ...
  M15: Passive verb + directional prep split (lines ≥65)        [was ≥50]
  M16: Deity appositive split (DEITY, DEITY → separate lines, ≥40 chars)
  M17: Obligation/expedient formula + "that" split
  M18: List-stacking for parallel triads (3+ "and X" items)     [NEW]
"""
...
def m1_aictp(lines):
    """AICTP on its own line when total > 42 chars. 'that' stays with content."""
    result = []
    for line in lines:
        handled = False
        for formula in AICTP_FORMULAS:
            if line.startswith(formula) and len(line) > 42:
                # Split so "that" goes with the content line
                if 'that ' in formula:
                    prefix = formula.split('that ')[0].rstrip()
                    content = 'that ' + line[len(formula):]
                else:
                    prefix = formula.rstrip()
                    content = line[len(formula):]
...
# ============================================================
# M6: ACCORDING-TO SPLIT  [v8: 58 → 72]
# ============================================================

def m6_according_to(lines):
    """', according to' splits on lines > 72 chars."""
    result = []
    for line in lines:
        if len(line) > 72:
...
# ============================================================
# M8: ", AND [SUBJECT PRONOUN]" SPLIT  [v8: 45-70 → 60-90]
# ============================================================

_AND_PRONOUN_PAT = re.compile(r', and (I |he |she |they |we |ye |it |thou )')

def m8_and_pronoun(lines):
    """', and [subject pronoun]' on lines 60-90, skipping triads."""
    result = []
    for line in lines:
        if 60 <= len(line) <= 90:
...
# ============================================================
# M9: SUBJECT-PREDICATE SPLIT AT MODAL VERBS  [v8: >70 → >85]
# ============================================================

def m9_subject_predicate(lines):
    """Long subject + modal verb on lines > 85: split before modal."""
    result = []
    for line in lines:
        if len(line) <= 85:
            result.append(line)
...
# ============================================================
# M10: LONG-LINE PATTERN-PRIORITY SPLIT  [v8: 70 → 90, 75 → 100]
# ============================================================
...
def _find_long_line_break(line, threshold=90):
    ...
    # Non-comma prep patterns (>100 only)  [v8: was >75]
    if len(line) > 100:
...
def m10_split_long(lines, threshold=90):
...
# ============================================================
# M15: PASSIVE VERB + DIRECTIONAL PREP SPLIT  [v8: ≥50 → ≥65]
# ============================================================

_EXCLUDED_PARTICIPLES = {
    # Motion verbs (original)
    'scattered', 'driven', 'led', 'turned', 'sent', 'taken',
    'cast', 'placed', 'put',
    # Phrasal verbs where the prep is the verb's complement
    'prepared', 'administered', 'delivered', 'restored',
    'given', 'brought', 'carried', 'shown', 'known',
}
...
def m15_passive_prep(lines):
    ...
    for line in lines:
        ...
        if m:
            ...
            if (participle not in _EXCLUDED_PARTICIPLES
...
_OBLIGATION_PAT = re.compile(
    r'^(.*(?:must needs be|must needs be expedient|is expedient|it behooveth))\s+(that\s+.+)$'
)

def m17_obligation_that(lines):
    """Split 'must needs be [expedient] that' / 'it is expedient that' before 'that'."""
    result = []
    for line in lines:
        if len(line) < 35:
            result.append(line)
            continue
        m = _OBLIGATION_PAT.match(line)
        if m:
            left, right = m.group(1), m.group(2)
            if len(left.strip()) >= 15 and len(right.strip()) >= 10:
                result.append(left)
                result.append(right)
                continue
        result.append(line)
...
# ============================================================
# M18: LIST-STACKING FOR PARALLEL TRIADS  [NEW in v8]
# ============================================================

def m18_list_stacking(lines):
    """Stack parallel list items vertically when 3+ ', and X' items appear.
    ...
    Only fires when:
      - 3 or more ', and ' separators in the line
      - Each item is short enough to be a list element (< 40 chars)
      - The line is long enough to benefit (> 40 chars)
    """
    result = []
    for line in lines:
        if len(line) <= 40:
        ...
        and_splits = list(re.finditer(r',\s+and\s+', line))
        if len(and_splits) < 2:
            # Need at least 2 separators = 3 items
            result.append(line)
            continue

        # Split at each ', and ' — keep comma on the preceding item
        parts = []
        prev_end = 0
        for m in and_splits:
            parts.append(line[prev_end:m.start()] + ',')
            prev_end = m.end()  # skip past ', and ' — next part starts with the item

        parts.append(line[prev_end:])

        # Validate: first part should be substantial, subsequent parts
        # should each be list-item sized (< 40 chars)
        if len(parts[0].strip()) < 5:
            result.append(line)
            continue

        all_items_short = all(len(p.strip()) < 40 for p in parts[1:])
        if not all_items_short:
            result.append(line)
            continue

        # Stack them: first part already has comma, rest are bare items
        result.append(parts[0])
        for p in parts[1:]:
            stripped = p.strip()
            # Prepend 'and ' since we split on ', and '
            result.append('and ' + stripped)
...
PIPELINE = [
    ...
    ('M10: long-line split',      lambda lines: m10_split_long(lines, 90)),
    ...
    ('M17: obligation+that',      m17_obligation_that),
    ('M18: list-stacking',        m18_list_stacking),
]
...
def print_stats(output, orig_blocks, output_blocks):
    ...
    print(f"Over 90: {sum(1 for l in lengths if l > 90)}")
    ...
    remaining = [l for l in out_lines if len(l) > 90]
    if remaining:
        print(f"\nRemaining > 90 ({len(remaining)}):")
        for l in remaining:
            print(f"  [{len(l)}] {l}")
